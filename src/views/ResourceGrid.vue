...
const featuredResources = computed(() => {
  ...
  return featuredResourcesData
    .filter(resource => {
      // Verificar si coincide con el filtro
      const matchesFilter = filter.value === 'Todos' || 
        (resource.formatNames && resource.formatNames.includes(filter.value))
    ...
    .map(resource => ({
      ...resource,
      image: null, // El componente ResourceCard mostrará un placeholder automáticamente
      isFeatured: true
    }))
})
...
const filteredResources = computed(() => {
  ...
  // Agregar los recursos destacados al principio si existen
  const featured = featuredResources.value
  return featured.length > 0 ? [...featured, ...filtered] : filtered
})
...
const handleEditResource = (resource) => {
  // No permitir editar recursos destacados (hardcoded)
...
const handleDeleteResource = (resource) => {
  // No permitir eliminar recursos destacados (hardcoded)
  if (resource.isFeatured) {
    return
  }
